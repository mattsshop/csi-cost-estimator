  ...
  */
  ...
  const handleGeminiHelp = async () => {
    ...
    try {
      ...
      if (error.message === "MISSING_API_KEY") {
        showToast("Gemini API Key missing. Please set GEMINI_API_KEY in your deployment environment.", "error");
      } else {
        showToast("Gemini AI failed to connect. Check your internet or API key.", "error");
      }
    } finally {
  ...
  const handleAIQuoteProcess = async (textOrFile: string | { data: string; mimeType: string }) => {
    ...
    try {
      ...
      if (error.message === "MISSING_API_KEY") {
        showToast("AI extraction requires a GEMINI_API_KEY in your deployment settings.", "error");
      } else {
        showToast("AI failed to parse the quote. Check your API key or try regular typing.", "error");
      }
    } finally {
  ...
  const handleAiDivisionPrompt = async (divisionId: string, prompt: string) => {
    ...
    try {
      ...
      if (error.message === "MISSING_API_KEY") {
        showToast("Gemini API Key is missing in this deployment.", "error");
      } else {
